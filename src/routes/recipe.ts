 ...
 *     description: |
 *       Fetch all the subcategories associated with a recipe based on its ID. This endpoint does not require any query parameters. 
 ...
 *     description: |
 *       Add new subcategories to an existing recipe by its ID. This endpoint accepts an array of subcategory IDs in the request body.
 ...
 *     description: |
 *       Remove a specific subcategory from a recipe by its `recipeId` and `subcategoryId`. This endpoint ensures that the subcategory is associated with the recipe before it can be removed. The operation is performed within a transaction to maintain data integrity.
 *     parameters:
 *       - name: id
 *         in: path
 ...
 *     description: |
 *       Fetch a list of all available images for a specific recipe identified by its `recipeId`. You can filter the images using pagination parameters `limit` and `page`. The `limit` defines the number of images per page (default is 10), and `page` defines the page number (default is 1).
 *     parameters:
 *       - name: id
 *         in: path
 ...
 *     description: |
 *       Add one or more images to a specific recipe identified by its `recipeId`. The request body should contain image data, which could include the image URL, type (e.g., thumbnail, full-size), and an optional caption.
 *     parameters:
 *       - name: id
 *         in: path
 ...
 *     description: |
 *       Removes a specific image from a recipe identified by its `recipeId` and `imageId`. The image will be deleted from the recipe and the associated cache will be invalidated.
 *     parameters:
 *       - name: id
 *         in: path
